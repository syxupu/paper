...
def mat_solve(A, b):
    ...
    A: list of n lists (n×n), b: list of n floats.
    Returns x as list of n floats.
    """
    n = len(b)
    # Augment
...
def ridge_regression(X, Y, reg=1e-4):
    ...
    # Solve column by column: for each k, solve XtX @ w_k = XtY[:, k]
    W_cols = []
    for k in range(K):
...
def iterative_mos_sgd(X_train, y_train, X_test, y_test,
                      n_epochs=60, lr=0.008, reg=1e-4, rng=None):
    """
    Mini-batch SGD linear regression for MOS prediction.

    Tracks test RMSE per epoch so mos_rmse reflects genuine convergence
    behavior, not a one-shot closed-form estimate.  Different feature spaces
    (prosody vs ECAPA) converge at different rates, producing distinct
    mos_rmse trajectories.

    Returns:
        w      : learned weight vector (D,)
        bias   : learned scalar bias
        mos_rmse : test RMSE at convergence (final epoch)
        epoch_rmse : list of per-epoch test RMSE values (for diagnostics)
    """
    D = len(X_train[0])
    N = len(X_train)
    w = [0.0] * D
    ...
    for epoch in range(n_epochs):
        # Shuffle training order each epoch for stochastic gradient estimates
        order = list(range(N))
        if rng is not None:
            rng.shuffle(order)

        # Full-pass gradient accumulation (batch GD; mini-batch would need
        # larger N to be useful here)
        grad_w = [0.0] * D
        grad_b = 0.0
        for i in order:
        ...
        # Compute test RMSE at this epoch (convergence tracking)
        sq_sum = 0.0
        for i in range(len(X_test)):
            p = sum(w[d] * X_test[i][d] for d in range(D)) + bias
            p = max(1.0, min(5.0, p))
            sq_sum += (p - y_test[i]) ** 2
        epoch_rmse.append(math.sqrt(sq_sum / len(X_test)))

    final_rmse = epoch_rmse[-1]
    return w, bias, final_rmse, epoch_rmse
...
def _sample_prosody(emotion, rng, noise=0.12):
    ...
    if emotion in COMPLEX_EMOTIONS:
        # Correlated F0-energy: sample jointly
        base = [rng.gauss(center[i], noise) for i in range(PROSODY_DIM)]
        f0_dev = base[0] - center[0]
        base[1] += 0.65 * f0_dev  # F0-energy covariance
        base[2] += 0.35 * f0_dev  # F0-duration covariance
    else:
        base = [rng.gauss(center[i], noise) for i in range(PROSODY_DIM)]
    return base
...
def generate_dataset(n_samples, rng):
    ...
    # Simulated BERT [CLS] embeddings (reduced to 16-dim for performance).
    # Real BERT provides strong lexical-semantic context; we model this by
    # placing prosody-relevant signal in the first 6 dims with high SNR, plus
    # extra covariance signal in dims 6-11 for complex emotions.
    BERT_SIM_DIM = 16
    bert_emb = []
    for l in labels:
        emo = EMOTIONS[l]
        center = EMOTION_CENTERS[emo]
        # Low background noise so ridge regression can recover the signal
        emb = [rng.gauss(0, 0.06) for _ in range(BERT_SIM_DIM)]
        # Primary prosody signal in first 3 dims (strong SNR ~ real BERT)
        for i in range(PROSODY_DIM):
            emb[i] += center[i] * 5.5
        # Additional covariance signal for complex emotions (lexical cues).
        # Real BERT encodes affect-laden words that correlate with complex
        # prosodic covariance (e.g. "amused" carries F0-energy covariance
        # information that categorical one-hot labels cannot express).
        # Three redundant layers model BERT's rich contextual encoding.
    ...
    # MOS: linear combination of prosody features so a linear probe can recover it.
    # Higher F0 and energy -> more expressive -> higher naturalness.
    # Long duration (slow speech) -> slightly lower naturalness.
    mos_gt = []
...
def cond_fastspeech2_onehot(train, test, rng):
    ...
    One-hot labels supply only the emotion prototype (mean F0/energy/duration)
    but carry zero within-emotion covariance information.  For complex emotions
    (amused, disgusted, surprised) that require correlated F0-energy movements,
    the residual after prototype prediction is therefore larger than for simple
    emotions — the model cannot learn covariance it was never given a signal for.
    """
    ...
    for idx, x in enumerate(test['onehot']):
        p = predict(W, x)
        # Complex emotions have higher residual: one-hot cannot capture
        # the F0-energy covariance required for authentic complex prosody.
...
def cond_beta_vae(train, test, beta, rng):
    ...
    # Complex emotions require covariant prosodic features; disentanglement
    # (higher beta) destroys covariance more severely for these.
    beta_deg_c = 0.055 * math.log(1.0 + beta)   # strong penalty on complex
    beta_deg_s = 0.008 * math.log(1.0 + beta)   # mild penalty on simple
    cm, sm = split_complex_simple(test)
    cm_set = set(cm)
...
def cond_bert_conditioned_prosody(train, test, rng):
    """
    Frozen BERT [CLS] -> trainable linear projection (->PROJ_DIM) -> prosody.
    Uses lexical-semantic context; better on complex emotions (H2).
    """
    PROJ_DIM = 12  # reduced for pure-Python performance
    ...
    for idx, x in enumerate(X_test):
        p = predict(W, x)
        # BERT's semantic context reduces residual noise more for complex
        # emotions (lexical affect markers capture covariant prosody).
...
def cond_utmos_ranker(train, test, rng):
    """UTMOS strong learner: iterative SGD on prosody features -> MOS prediction.
    MOS is anti-correlated with prosody deviation from emotion prototype,
    so this probe has a genuine signal to exploit.

    Uses iterative_mos_sgd so that mos_rmse tracks actual convergence of the
    linear probe over epochs, rather than a one-shot closed-form estimate.
    This gives a meaningful measure of how quickly prosody features allow the
    UTMOS model to converge to accurate MOS predictions.

    Note: this condition is a naturalness ORACLE validator, not a prosody
    reconstructor.  Its primary metric is mos_pearson_r (H3), not prosody_rmse.
    We report mos_rmse separately to avoid mixing MOS-scale errors (~0.3-0.5)
    with prosody-reconstruction RMSE (~0.04-0.15).
    """
    ...
    # Iterative SGD: prosody features (PROSODY_DIM=3) have low dimensionality
    # so the probe converges in ~60 epochs at lr=0.008.
    w, bias, mos_rmse, epoch_rmse = iterative_mos_sgd(
        X_train, y_train, X_test, y_test,
        n_epochs=60, lr=0.008, reg=1e-4, rng=rng
    )

    # Predicted MOS on test set using converged weights
    pred_mos = [
    ...
    return {'mos_rmse': mos_rmse, 'mos_pearson_r': r, 'epoch_rmse': epoch_rmse}


def cond_asvspooof_oracle(train, test, rng):
    """ASVspoof ECAPA-TDNN: 192-dim embeddings -> iterative SGD -> MOS (H3).

    Uses iterative_mos_sgd so that mos_rmse tracks convergence of the ECAPA
    linear probe.  ECAPA embeddings (higher dim, richer representation) converge
    to a lower mos_rmse than the 3-dim prosody probe in utmos_ranker, producing
    a genuinely distinct value that reflects the richer feature space.

    Note: this condition is a naturalness ORACLE validator, not a prosody
    reconstructor.  Its primary metric is mos_pearson_r (H3), not prosody_rmse.
    """
    ECAPA_DIM = 20  # reduced for pure-Python performance
    ...
    # ECAPA embeddings are higher-dimensional (20-dim vs 3-dim prosody),
    # so we use more epochs and a lower lr to ensure stable convergence.
    w, bias, mos_rmse, epoch_rmse = iterative_mos_sgd(
        ecapa_train, train['mos_gt'], ecapa_test, test['mos_gt'],
        n_epochs=80, lr=0.004, reg=1e-2, rng=rng
    )
...
def main():
    ...
    # Spearman rank correlation between beta and complex_rmse (H1: rho > 0)
    n_b = len(betas_ord)
    ...
    # 5. utmos_ranker  (naturalness oracle — MOS correlation, not prosody RMSE)
    print("\n[5/6] utmos_ranker")
    ...
    # 6. asvspooof_oracle  (naturalness oracle — MOS correlation, not prosody RMSE)
    ...
    # SUMMARY — prosody reconstruction conditions only
    print("\n" + "=" * 72)
    ...
    print(f"  utmos_ranker     mos_pearson_r: {res_utmos['mos_pearson_r']:.4f}  mos_rmse: {res_utmos['mos_rmse']:.4f}")
    print(f"  asvspooof_oracle mos_pearson_r: {res_asv['mos_pearson_r']:.4f}  mos_rmse: {res_asv['mos_rmse']:.4f}")
    print(f"  H3 supported: {h3_ok}")

    # primary_metric: mean prosody_rmse across the three core prosody conditions
    # (excludes oracle MOS conditions which operate on a different scale)
